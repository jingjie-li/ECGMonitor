 ...
 *          GotoXY则需加入对Printf.h的文件包含( 函数移至液晶函数
 *          库)。
 *          
 * 创建标识：   刘中原20110729
 * 
 ...
 * 
 * 修改标识：   刘中原20110827
 * 修改描述：   GotoXY，不应该位于此文件；把GotoXY函数移动到液晶。文
 *              件更名为PutChar.c 更符合文件内容
**----------------------------------------------------------------*/
...
* 入口参数：ch: 待发送的字符  
* 出口参数：发出的字符
* 说    明: printf函数会调用该函数作为底层输出。这里输出字符到12864的液晶这
            printf的结果将打印到12864的液晶上。若修改该函数，将字
            符以其他方式输出，如异步串口通向PC的超级终端， printf的结果将显
            示在超级终端里。
****************************************************************************/
int putchar(int ch)
{
