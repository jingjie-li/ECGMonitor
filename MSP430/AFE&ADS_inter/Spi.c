 ...
 * 	        最后是写入同时返回读取的值函数SpiWriteData(char c)，
...
/***********************************宏定义*********************************/
#define UxCTL   U1CTL
#define UxRCTL  U1RCTL
#define UxTCTL  U1TCTL

#define UxBR0   U1BR0
#define UxBR1   U1BR1
#define UxMCTL  U1MCTL

#define UxRXBUF U1RXBUF
#define UxTXBUF U1TXBUF
...
#define USARTxTX_VECTOR USART1TX_VECTOR
#define USARTxRX_VECTOR USART1RX_VECTOR

#define USPI3ON P5SEL |= 0X0E           //SPI 0的第二功能打开(三线模式)
#define USPI4ON P5SEL |= 0X0F           //SPI 0的第二功能打开(四线模式)
/**************************************************************************/
...
char SpiMasterInit(long baud,char dataBits,char mode,char clkMode)
{
    ...
    else
    {
      ...
      brclk = 8000000;              //波特率发生器时钟频率=SMCLK(8MHz)
    }
    
    //------------------------设置波特率-------------------------   
    if(baud < 300||baud > 8000000)   //波特率超出范围
    {
        return 0;
    }
    //设置波特率寄存器
...
/****************************************************************************
* 名    称：UartRx
* 功    能：串口接收中断，每接收到1一个字节会发生一次中断
****************************************************************************/
#pragma vector=USARTxRX_VECTOR
__interrupt void Uart_spiRx()
{
    RxFlag=1;
...
/****************************************************************************
* 名    称：UartTx
* 功    能：串口发送中断，每发完1个字节会发生一次中断
****************************************************************************/
#pragma vector=USARTxTX_VECTOR
__interrupt void Uart_spiTx ()
{
    TxFlag=1;
    __low_power_mode_off_on_exit();
}
