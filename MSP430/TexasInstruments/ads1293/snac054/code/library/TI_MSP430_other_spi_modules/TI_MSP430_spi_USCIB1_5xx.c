...
//******************************************************************************
// Support for 552x USCI_B1
//******************************************************************************
#if TI_ADS1293_SER_INTF == TI_ADS1293_SER_INTF_USCIB1_5xx
...
void TI_ADS1293_SPISetup(void)
{
  ...
  TI_ADS1293_SPI_USCIB1_PxSEL1 |= TI_ADS1293_SPI_USCIB1_SIMO
                                   | TI_ADS1293_SPI_USCIB1_SOMI;              
  TI_ADS1293_SPI_USCIB1_PxSEL2 |= TI_ADS1293_SPI_USCIB1_UCLK;
                                                                               // SPI option select
  TI_ADS1293_SPI_USCIB1_PxDIR1 |= TI_ADS1293_SPI_USCIB1_SIMO;
  TI_ADS1293_SPI_USCIB1_PxDIR2 |= TI_ADS1293_SPI_USCIB1_UCLK;
...
void TI_ADS1293_SPIWriteReg(uint8_t addr, uint8_t value)
{
  ...
  UCB1TXBUF = inst;                                                            // Send lower register address
  
  while (!(UCB1IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCB1TXBUF = value;                                                           // Send data value  
...
void TI_ADS1293_SPIAutoIncWriteReg(uint8_t addr, uint8_t *buffer, uint8_t count)
{
  ...
  {
    while (!(UCB1IFG&UCTXIFG));                                                // Wait for TXBUF ready
    UCB1TXBUF = *(buffer+i);                                                   // Send data value  
  }
...
void TI_ADS1293_SPIAutoIncReadReg(uint8_t addr, uint8_t *buffer, uint8_t count)
{
  ...
  while (!(UCB1IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCB1TXBUF = inst;                                                            // Send register address  
...
void TI_ADS1293_SPIStreamReadReg(uint8_t *buffer, uint8_t count)
{
  ...
  for(i=0; i<count; i++)
  {
    ...
    while (UCB1STAT & UCBUSY);                                                 // Wait for TX complete  
    *(buffer+i) = UCB1RXBUF;                                                   // Read register data    
  }
