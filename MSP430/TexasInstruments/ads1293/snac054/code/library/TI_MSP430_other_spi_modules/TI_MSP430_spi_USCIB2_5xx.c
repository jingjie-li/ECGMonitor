...
//******************************************************************************
// Support for 552x USCI_B2
//******************************************************************************
#if TI_ADS1293_SER_INTF == TI_ADS1293_SER_INTF_USCIB2_5xx
...
void TI_ADS1293_SPISetup(void)
{
  ...
  TI_ADS1293_SPI_USCIB2_PxSEL1 |= TI_ADS1293_SPI_USCIB2_SIMO
                                   | TI_ADS1293_SPI_USCIB2_SOMI;              
  TI_ADS1293_SPI_USCIB2_PxSEL2 |= TI_ADS1293_SPI_USCIB2_UCLK;
                                                                               // SPI option select
  TI_ADS1293_SPI_USCIB2_PxDIR1 |= TI_ADS1293_SPI_USCIB2_SIMO;
  TI_ADS1293_SPI_USCIB2_PxDIR2 |= TI_ADS1293_SPI_USCIB2_UCLK;
...
void TI_ADS1293_SPIWriteReg(uint8_t addr, uint8_t value)
{
  ...
  UCB2TXBUF = inst;                                                            // Send lower register address
  
  while (!(UCB2IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCB2TXBUF = value;                                                           // Send data value  
...
void TI_ADS1293_SPIAutoIncWriteReg(uint8_t addr, uint8_t *buffer, uint8_t count)
{
  ...
  {
    while (!(UCB2IFG&UCTXIFG));                                                // Wait for TXBUF ready
    UCB2TXBUF = *(buffer+i);                                                   // Send data value  
  }
...
void TI_ADS1293_SPIAutoIncReadReg(uint8_t addr, uint8_t *buffer, uint8_t count)
{
  ...
  while (!(UCB2IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCB2TXBUF = inst;                                                            // Send register address  
...
void TI_ADS1293_SPIStreamReadReg(uint8_t *buffer, uint8_t count)
{
  ...
  for(i=0; i<count; i++)
  {
    ...
    while (UCB2STAT & UCBUSY);                                                 // Wait for TX complete  
    *(buffer+i) = UCB2RXBUF;                                                   // Read register data    
  }
