...
//******************************************************************************
// Support for FRAM 57xx eUSCI_A0
//******************************************************************************
#if TI_ADS1293_SER_INTF == TI_ADS1293_SER_INTF_eUSCIA0_FR57xx
...
void TI_ADS1293_SPISetup(void)
{
  ...
  UCA0CTLW0 |= UCMST+UCCKPL+UCMSB+UCSYNC;                                      // 3-pin, 8-bit SPI master  
  UCA0CTLW0 |= UCSSEL_2;                                                       // SMCLK
  UCA0BRW = 0x04;                                                              // UCLK/4
  UCA0MCTLW = 0;
  
  TI_ADS1293_SPI_eUSCIA0_SIMO_PxSEL1 |= TI_ADS1293_SPI_eUSCIA0_SIMO;           // SPI option select
  TI_ADS1293_SPI_eUSCIA0_SOMI_PxSEL1 |= TI_ADS1293_SPI_eUSCIA0_SOMI;              
  TI_ADS1293_SPI_eUSCIA0_UCLK_PxSEL1 |= TI_ADS1293_SPI_eUSCIA0_UCLK;
  
  TI_ADS1293_SPI_eUSCIA0_SIMO_PxSEL0 &= ~TI_ADS1293_SPI_eUSCIA0_SIMO;            
  TI_ADS1293_SPI_eUSCIA0_SOMI_PxSEL0 &= ~TI_ADS1293_SPI_eUSCIA0_SOMI;              
  TI_ADS1293_SPI_eUSCIA0_UCLK_PxSEL0 &= ~TI_ADS1293_SPI_eUSCIA0_UCLK;  
                                                                               
  TI_ADS1293_SPI_eUSCIA0_SIMO_PxDIR |= TI_ADS1293_SPI_eUSCIA0_SIMO;            // SPI TXD out direction
  TI_ADS1293_SPI_eUSCIA0_UCLK_PxDIR |= TI_ADS1293_SPI_eUSCIA0_UCLK;
...
void TI_ADS1293_SPIAutoIncWriteReg(uint8_t addr, uint8_t *buffer, uint8_t count)
{
  ...
  {
    while (!(UCA0IFG&UCTXIFG));                                                // Wait for TXBUF ready
    UCA0TXBUF = *(buffer+i);                                                   // Send data value  
  }
...
void TI_ADS1293_SPIAutoIncReadReg(uint8_t addr, uint8_t *buffer, uint8_t count)
{
  ...
  while (!(UCA0IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCA0TXBUF = inst;                                                            // Send register address  
...
void TI_ADS1293_SPIStreamReadReg(uint8_t *buffer, uint8_t count)
{
  ...
  for(i=0; i<count; i++)
  {
    ...
    while (UCA0STATW & UCBUSY);                                                // Wait for TX complete  
    *(buffer+i) = UCA0RXBUF;                                                   // Read register data    
  }
