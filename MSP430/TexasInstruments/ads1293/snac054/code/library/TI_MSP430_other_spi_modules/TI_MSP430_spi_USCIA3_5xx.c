...
//******************************************************************************
// Support for 552x USCI_A3
//******************************************************************************
#if TI_ADS1293_SER_INTF == TI_ADS1293_SER_INTF_USCIA3_5xx
...
void TI_ADS1293_SPISetup(void)
{
  ...
  UCA3BR1 = 0;
  UCA3MCTL = 0;
  TI_ADS1293_SPI_USCIA3_PxSEL1 |= TI_ADS1293_SPI_USCIA3_SIMO
                                   | TI_ADS1293_SPI_USCIA3_SOMI;              
  TI_ADS1293_SPI_USCIA3_PxSEL2 |= TI_ADS1293_SPI_USCIA3_UCLK;
                                                                               // SPI option select
  TI_ADS1293_SPI_USCIA3_PxDIR1 |= TI_ADS1293_SPI_USCIA3_SIMO;
  TI_ADS1293_SPI_USCIA3_PxDIR2 |= TI_ADS1293_SPI_USCIA3_UCLK;
...
void TI_ADS1293_SPIWriteReg(uint8_t addr, uint8_t value)
{
  ...
  UCA3TXBUF = inst;                                                            // Send lower register address
  
  while (!(UCA3IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCA3TXBUF = value;                                                           // Send data value  
...
void TI_ADS1293_SPIAutoIncWriteReg(uint8_t addr, uint8_t *buffer, uint8_t count)
{
  ...
  {
    while (!(UCA3IFG&UCTXIFG));                                                // Wait for TXBUF ready
    UCA3TXBUF = *(buffer+i);                                                   // Send data value  
  }
...
void TI_ADS1293_SPIAutoIncReadReg(uint8_t addr, uint8_t *buffer, uint8_t count)
{
  ...
  while (!(UCA3IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCA3TXBUF = inst;                                                            // Send register address  
...
void TI_ADS1293_SPIStreamReadReg(uint8_t *buffer, uint8_t count)
{
  ...
  for(i=0; i<count; i++)
  {
    ...
    while (UCA3STAT & UCBUSY);                                                 // Wait for TX complete  
    *(buffer+i) = UCA3RXBUF;                                                   // Read register data    
  }
