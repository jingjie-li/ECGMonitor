...
//******************************************************************************
// Support for 552x USCI_A2
//******************************************************************************
#if TI_ADS1293_SER_INTF == TI_ADS1293_SER_INTF_USCIA2_5xx
...
void TI_ADS1293_SPISetup(void)
{
  ...
  UCA2BR1 = 0;
  UCA2MCTL = 0;
  TI_ADS1293_SPI_USCIA2_PxSEL1 |= TI_ADS1293_SPI_USCIA2_SIMO
                                   | TI_ADS1293_SPI_USCIA2_SOMI;              
  TI_ADS1293_SPI_USCIA2_PxSEL2 |= TI_ADS1293_SPI_USCIA2_UCLK;
                                                                               // SPI option select
  TI_ADS1293_SPI_USCIA2_PxDIR1 |= TI_ADS1293_SPI_USCIA2_SIMO;
  TI_ADS1293_SPI_USCIA2_PxDIR2 |= TI_ADS1293_SPI_USCIA2_UCLK;
...
void TI_ADS1293_SPIWriteReg(uint8_t addr, uint8_t value)
{
  ...
  UCA2TXBUF = inst;                                                            // Send lower register address
  
  while (!(UCA2IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCA2TXBUF = value;                                                           // Send data value  
...
void TI_ADS1293_SPIAutoIncWriteReg(uint8_t addr, uint8_t *buffer, uint8_t count)
{
  ...
  {
    while (!(UCA2IFG&UCTXIFG));                                                // Wait for TXBUF ready
    UCA2TXBUF = *(buffer+i);                                                   // Send data value  
  }
...
void TI_ADS1293_SPIAutoIncReadReg(uint8_t addr, uint8_t *buffer, uint8_t count)
{
  ...
  while (!(UCA2IFG&UCTXIFG));                                                  // Wait for TXBUF ready
  UCA2TXBUF = inst;                                                            // Send register address  
...
void TI_ADS1293_SPIStreamReadReg(uint8_t *buffer, uint8_t count)
{
  ...
  for(i=0; i<count; i++)
  {
    ...
    while (UCA2STAT & UCBUSY);                                                 // Wait for TX complete  
    *(buffer+i) = UCA2RXBUF;                                                   // Read register data    
  }
