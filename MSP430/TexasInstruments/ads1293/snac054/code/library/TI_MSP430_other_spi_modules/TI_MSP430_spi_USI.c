...
//******************************************************************************
// Support for G2x31 USI
//******************************************************************************
#if TI_ADS1293_SER_INTF == TI_ADS1293_SER_INTF_USI
//------------------------------------------------------------------------------
...
void TI_ADS1293_SPISetup(void)
{
  ...
  USISRL = 0x00;                                                               // Ensure SDO low 
  USICNT = 1;                                                                  // USI bit count
...
void TI_ADS1293_SPIAutoIncReadReg(uint8_t addr, uint8_t *buffer, uint8_t count)
{
  ...
  for(i=0; i < count; i++)
  {
    USISRL = 0;                                                                // Dummy write to read data
    USICNT = 8;                                                                // Send it
    while (!(USICTL1&USIIFG));                                                 // Wait for TX to finish
    *(buffer+i) = USISRL;                                                      // Read data
  }
...
void TI_ADS1293_SPIStreamReadReg(uint8_t *buffer, uint8_t count)
{
  ...
  for(i=0; i<count; i++)
  {
    ...
    USICNT = 8;                                                                // Send it
    while (!(USICTL1&USIIFG));                                                 // Wait for TX to finish
    *(buffer+i) = USISRL;                                                      // Read register data    
  }
